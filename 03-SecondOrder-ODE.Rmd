...
In the above, example, we can solve for $A$ directly from resulting equation after plugging in $y_p$. Indeed, we solve for $A$ from the equation by putting $x=0$. 
...
```
CoefCosine := subs([sin(x) = 0, cos(x) = 1], eqnB); # get an equation from coefficients of cosine
CoefSine := subs([sin(x) = 1, cos(x) = 0], eqnB); # get an equation from coefficients of sine
...
The output is
$$\color{blue}{\{a = \frac{1}{10}, b = -\frac{7}{50}, c = -\frac{1}{5}, d
 = -\frac{1}{50}\}
}.$$

Let's versify that the solution.
```
