...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{f(t):=exp(a*t)*(A*cos(b*t)+B*sin(b*t));}
\end{Highlighting}
\end{Shaded}
...
\end{pmatrix}P^{-1},
\]
where \(P=\begin{pmatrix}  \vec{u} & \vec{v} \end{pmatrix}\).
...
In Maple, eigenvalues, eigenvectors can be found using the command \texttt{Eigenvectors} which is supported by the package \texttt{LinearAlgebra}.

To construct a matrix or vector in Maple, one can use the \texttt{Matrix()} or \texttt{Vector()} command, or the shortcut notation \texttt{\textless{}...\textgreater{}}.
...
Load the package \texttt{LinearAlgebra}.
...
\hypertarget{solving-linear-system-of-ode}{%
\section{Solving Linear System of ODE}\label{solving-linear-system-of-ode}}

In Maple, to solve a linear system of first-order differential equations, one can use the command \texttt{dsolve}. Note that in Maple, the matrix multiplication operator is the \texttt{.} symbol instead of the \texttt{*} symbol. The \texttt{*} symbol is to be used for scalar multiplication.
